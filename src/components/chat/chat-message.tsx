...
function MarkdownContent({ content }: { content: string }) {
  ...
  return (
    <div>
      ...
      >
        <ReactMarkdown
          remarkPlugins={[remarkGfm, remarkMath, remarkInlineExtensions]}
          rehypePlugins={[
          ...
          components={{
            ...
            th: ({ children, ...props }) => (
              <th className="border border-border/70 bg-muted/90 px-4 py-2.5 text-left font-semibold text-foreground" {...props}>{children}</th>
            ),
            td: ({ children, ...props }) => (
              <td className="border border-border/50 px-4 py-2 text-foreground/90" {...props}>{children}</td>
            ),
            pre: ({ children, ...props }) => {
