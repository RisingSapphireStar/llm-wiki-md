...
## About This Fork (Markdown Rendering Plus)

This is a **fork of [nashsu/llm_wiki](https://github.com/nashsu/llm_wiki)** focused on **enhancing the Markdown reading experience**. All original features (ingest, knowledge graph, chat agent, etc.) are preserved; this fork adds the following rendering improvements on top of the upstream `WikiReader` / `ChatMessage` react-markdown pipeline:

- **Code block syntax highlighting** - integrated `rehype-highlight` with a language label and one-click copy toolbar
- **GitHub-style alert blocks** - supports `> [!NOTE]`, `[!WARNING]`, `[!TIP]`, ... (13 types) via a custom `rehype-alerts` plugin
- **Table styling** - larger font, clear header background, zebra striping, horizontal scroll without overflow
- **Frontmatter property card** - renders page metadata (tags / date / author) as a card at the top of the article body
- **TOC auto-generation** - `remark-toc` injects an anchor-linked table of contents under a `## TOC` / `## 目录` heading
- **Inline extensions** - supports `==highlight==`, `~subscript~`, `^superscript^` via a custom remark plugin
- **KaTeX fault tolerance** - `strict: false` + `throwOnError: false` so malformed formulas degrade gracefully instead of blanking out

The upstream Milkdown editor pipeline is intentionally left untouched to keep editing stability. See [`miaoda-code-llm_wiki Markdown 渲染升级方案.md.md`](./miaoda-code-llm_wiki%20Markdown%20渲染升级方案.md.md) for the full design document.
