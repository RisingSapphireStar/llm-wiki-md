...
- **多格式文档解析** — 支持 PDF、Office 文档、EPUB/MOBI、图片、音视频和网页剪藏，并提供内置、云端或本地 MinerU PDF 处理
...
## 关于本 Fork（Markdown 渲染增强版）

这是 **[nashsu/llm_wiki](https://github.com/nashsu/llm_wiki) 的 Fork**，专注于**增强 Markdown 阅读体验**。所有原版功能（文档导入、知识图谱、聊天 Agent 等）完整保留；在此基础上，针对上游 `WikiReader` / `ChatMessage` 的 react-markdown 渲染管线新增以下增强：

- **代码块语法高亮** —— 接入 `rehype-highlight`，并附语言标签与一键复制工具栏
- **GitHub 风格告警块** —— 支持 `> [!NOTE]`、`[!WARNING]`、`[!TIP]` 等 13 种类型，通过自定义 `rehype-alerts` 插件实现
- **表格样式优化** —— 加大字号、加深表头背景、斑马纹、超长表格横向滚动不溢出
- **Frontmatter 属性卡片** —— 在正文顶部以卡片形式展示页面元数据（tags / date / author）
- **TOC 目录自动生成** —— 通过 `remark-toc` 在 `## TOC` / `## 目录` 标题下注入带锚点的目录
- **内联扩展语法** —— 支持 `==高亮==`、`~下标~`、`^上标^`（自定义 remark 插件）
- **KaTeX 公式容错** —— 开启 `strict: false` + `throwOnError: false`，语法有瑕疵的公式降级渲染而非整块空白

为保持编辑稳定性，上游 Milkdown 编辑器管线有意保持原样未动。完整设计文档见 [`miaoda-code-llm_wiki Markdown 渲染升级方案.md.md`](./miaoda-code-llm_wiki%20Markdown%20渲染升级方案.md.md)。
